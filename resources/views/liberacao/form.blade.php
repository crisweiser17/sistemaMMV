...
@php
    $itensIniciais = $liberacao->exists
        ? $liberacao->itens->map(fn ($i) => $i->only(['id','numero_item','cod_mmv','ni','descricao','quantidade','unidade_id','material_cliente','prazo_entrega_item','descricao_cliente','observacoes']))->values()
        : [];
    $template = ['id' => null, 'cod_mmv' => '', 'ni' => '', 'descricao' => '', 'quantidade' => '', 'unidade_id' => '', 'material_cliente' => '', 'prazo_entrega_item' => '', 'descricao_cliente' => '', 'observacoes' => ''];
@endphp

@section('content')
<form method="POST" action="{{ $liberacao->exists ? route('liberacao.update', $liberacao) : route('liberacao.store') }}"
      x-data="itemsRepeater(@js($itensIniciais), @js($template))"
    ...
    <x-card title="Dados do PI" class="mb-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <x-input label="NF Cliente" name="nf_cliente" :value="$liberacao->nf_cliente" />
